(function() {
  ...
  if (!document.body.getBoundingClientRect || !document.body.querySelectorAll || !window.postMessage) {
    return;
  }
  ...
  function showDialog(iframe) {
    ...
    function position() {
      ...
      var left = iframePos.left + Math.round(iframePos.width / 2) - Math.round(divPos.width / 2);
      if (left < sl) left = sl;
